...
public class MainActivity5 extends AppCompatActivity {
    ...
    private void configureRadioGroup() {
        opcionGrupo.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                ...
                configureMode();
                enableInputs(); // Habilita los EditText cuando se selecciona un RadioButton
            }
        });
    }
    ...
    private void calculateResult() {
        ...
        String valueB = campoB.getText().toString().trim();
        ...
        if (!validateInput(valueA) || !validateInput(valueB)) {
        ...
        if (!modoUnBit) {
            ...
            if (valueB.length() < 8) {
                valueB = String.format("%-8s", valueB).replace(' ', '0');
                campoB.setText(valueB);
            }
        }

        if (modoUnBit) {
            int bitA = Integer.parseInt(valueA);
            int bitB = Integer.parseInt(valueB);
            int result = bitA ^ bitB; // Operación XOR para un bit
            ...
            for (int i = 0; i < 8; i++) {
                ...
                int bitB = Character.getNumericValue(valueB.charAt(i));
                result.append(bitA ^ bitB); // Operación XOR para 8 bits
            }
    ...
    private void configureFocusChangeListeners() {
        ...
        campoB.setOnFocusChangeListener(focusChangeListener);
    }
}
