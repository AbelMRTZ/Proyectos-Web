...
class ExpressServer {
  ...
  // OpenAPI dynamic router (maneja todas las rutas declaradas en openapi.yaml)
  this.app.use(openApiRouter());

  // Programar cierre automático cada hora
  setInterval(async () => {
    try {
      await cerrarFichajesVencidos();
    } catch (error) {
  ...
  setTimeout(async () => {
    try {
      ...
      console.error('Error en cierre inicial:', error);
    }
  }, 5000);
  ...
  launch() {
    ...
    try {
      ...
      this.server.listen(this.port, () => {
        ...
        // LOG ADICIONAL: Mostrar rutas disponibles
        console.log('\n📋 Rutas disponibles:');
        console.log('   GET    /hello');
        console.log('   GET    /openapi');
        console.log('   GET    /api-docs');
        console.log('   GET    /health');
        console.log('   GET    /fichajes/activo');
        console.log('   GET    /fichajes/hoy');
        console.log('   POST   /fichajes/iniciar');
        console.log('   PUT    /fichajes/finalizar/:id');
        console.log('   POST   /usuarios/login');
        console.log('   POST   /apikey');
        console.log('   ... y todas las demás rutas de OpenAPI.yaml\n');
      });
